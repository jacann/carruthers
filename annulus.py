...
def main():
    with xr.open_dataset("products/WFI_DRK_ANNULUS_MEDIANS.nc") as ds:
        wfi_drk_ds = ds
    with xr.open_dataset("products/WFI_SCI_ANNULUS_MEDIANS copy.nc") as ds:
        wfi_sci_ds = ds
    ...
    p1 = ax1.scatter(wfi_sci_ds.time, wfi_sci_ds["annulus_median_full"], label="WFI SCI", s=2, color="red", alpha=0.5)
    p2 = ax1.scatter(nfi_sci_ds.time, nfi_sci_ds["annulus_median_full"], label="NFI SCI", s=2, color="green",alpha=0.5)
    ax1.set_xlabel("Time")
